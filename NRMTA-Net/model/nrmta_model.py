...
from model.encoder_net import Encoder_Net
...
class NRMTANetModel(BaseModel):
    """NRMTA-Net: Noise-Robust Multi-Task Attention Network."""

    def __init__(self, args):
        super(NRMTANetModel, self).initialize(args)
        ...
        self.backbone_type = "dual_stream"
        ...
        if self.args.img_size == 64:
            ...
            raise ValueError("Only img_size 64/112/128/224 supported for NRMTA-Net decoder.")
    ...
    def forward_G(self, epoch):
        ...
        # FIX: NRMTANetModel has no self.training; optimize_params() calls forward_G only in training.
        if self.use_arcface:
    ...
    def save_networks(self, epoch, logdir):
        super(NRMTANetModel, self).save_networks(epoch, logdir)
