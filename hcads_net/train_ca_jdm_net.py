    ...
    try:
        ...
        class SummaryWriter:  # type: ignore
            def __init__(self, log_dir: str = None, **kwargs):
                self._logdir = log_dir or os.getcwd()
                self.file_writer = _NoOpFileWriter(self._logdir)
...
from model.hcads_net import HCADSNet
...
def validate_model(model, test_loader, args):
    """Validation for dual-stream emotion+landmark model."""
    ...
    with torch.inference_mode():
        for data_val in tqdm(test_loader):
            ...
            fer_embed, _ = model.backbone(img)
            if getattr(model, "use_arcface", False):
                logits = model.c_layer(fer_embed, None)
            else:
                logits = model.c_layer(fer_embed)

            if use_tta:
                img_flip = torch.flip(img, dims=[3])
                fer_embed_flip, _ = model.backbone(img_flip)
                if getattr(model, "use_arcface", False):
                    logits_flip = model.c_layer(fer_embed_flip, None)
                else:
                    logits_flip = model.c_layer(fer_embed_flip)
    ...
    if backbone_was_training is not None and backbone_was_training and hasattr(model, 'backbone'):
...
def parse_args():
    ...
    parser.add_argument("--img_size", default=112, type=int, choices=[112, 224])
    ...
    parser.add_argument("--save_dir", default="runs_dual_stream_el", type=str)
    ...
    # Model selection (backbone-only, dual_stream)
    parser.add_argument(
        "--model_type",
        default="dual_stream_el",
        type=str,
        choices=["dual_stream_el"],
        help="Dual-stream (emotion + landmarks), backbone-only (no GAN/VAE)",
    )
    parser.add_argument(
        "--backbone",
        default="dual_stream",
    ...
    parser.add_argument("--raf_train_csv", default="Dataset\\RAF-DB\\train.csv", type=str)
    parser.add_argument("--raf_val_csv", default="Dataset\\RAF-DB\\test.csv", type=str)
    # parser.add_argument(
    #     "--data_path",
...
def train(writer, logger, args):
    ...
    def _load_best_from_dir(resume_dir_path):
        loaded_any = False
        backbone = getattr(model, "backbone", None)
        if backbone is not None:
            ckpt_path = os.path.join(resume_dir_path, "best_net_backbone.pth")
            if os.path.isfile(ckpt_path) and _try_load_state_dict(backbone, ckpt_path, "backbone"):
                loaded_any = True

        c_layer = getattr(model, "c_layer", None)
        if c_layer is not None:
            ckpt_path = os.path.join(resume_dir_path, "best_net_c_layer.pth")
            if os.path.isfile(ckpt_path):
                loaded_any = loaded_any or _try_load_state_dict(c_layer, ckpt_path, "c_layer")
        return loaded_any

    # If we have a resume directory, try common filenames and model-specific loader
    if not resumed and resume_dir:
        resumed = _load_best_from_dir(resume_dir)

        # Try common files
        if not resumed:
            enc_candidates = [
                os.path.join(resume_dir, 'best_model.pth'),
                os.path.join(resume_dir, 'best_backbone.pth'),
                os.path.join(resume_dir, 'best_net_backbone.pth'),
                os.path.join(resume_dir, 'best_backbone_state.pth'),
            ]
            for p in enc_candidates:
                if os.path.isfile(p) and hasattr(model, 'backbone'):
                    if _try_load_state_dict(model.backbone, p, 'backbone'):
                        resumed = True
                        break
    ...
    for epoch in range(args.num_epoch):
        for step, data_batch in tqdm(enumerate(train_loader)):
            ...
            if epoch == args.freeze_fld_epoch and step == 0 and epoch > 0:
                if hasattr(model, 'backbone') and hasattr(model.backbone, 'fld_backbone'):
                    logger.info("Freezing FLD branch (MobileFaceNet)...")
                    for param in model.backbone.fld_backbone.parameters():
            ...
            if total_steps % args.val_step == 0:
                ...
                if acc > best_acc:
                    ...
                    if hasattr(model, 'save_networks'):
                        ...
                        torch.save(model.backbone.state_dict(), os.path.join(writer.file_writer.get_logdir(), "best_backbone.pth"))
