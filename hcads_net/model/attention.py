...
class HeteroCoAttentionModule(nn.Module):
    ...
    def __init__(self, fer_channels: int, fld_channels: int, e_ratio: float = 0.2, scam_kernel: int = 7, spatial_size: int = None):
        ...
        # Zero-initialized parameters for residual connection to preserve pre-trained features
        self.gamma_fer = nn.Parameter(torch.zeros(1))
        self.gamma_fld = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, f_fer: torch.Tensor, f_fld: torch.Tensor):
        ...
        # CRITICAL FIX: Compute mask_fld directly from f_fld, NOT f_fld_aligned.
        # f_fld_aligned depends on self.fld_to_fer, which is randomly initialized.
        # If we use f_fld_aligned, mask_fld will be random at initialization, destroying FER features!
        mask_fld = torch.mean(f_fld.pow(2), dim=1, keepdim=True)
        ...
        # f_fld might have different spatial dimensions if not aligned, but in our architecture they match.
        # Just in case, interpolate mask_fld to match mask_fer's spatial dimensions.
        if mask_fld.shape[2:] != mask_fer.shape[2:]:
            mask_fld = F.interpolate(mask_fld, size=mask_fer.shape[2:], mode='bilinear', align_corners=False)
        ...
        # 残差 + 反向对齐 (Zero-initialized to preserve pre-trained features)
        out_fer = f_fer + self.gamma_fer * f_mix
        out_fld = f_fld + self.gamma_fld * self.fer_to_fld(l_mix)
